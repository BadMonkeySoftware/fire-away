...
_M.keys = {
    ...
    button1 = {type = 'action'},
    buttonA = {type = 'action'}, -- Press Trakpad-- Switch map movement/cannon control on Apple TV remote by pressing the touchpad
    buttonX = {type = 'action'}, -- Pause/play -- Apple TV, play/pause button to fire the cannnon
    space = {type = 'action'},
    enter = {type = 'action'},
...
function _M.setSelectionEffect(button, isSelected)
    ...
    if isSelected then
        ...
        -- self.activeVisualButtonOutline:setFillColor(unpack(utilities:hex2rgb('#80ff80', 0.2)))
        self.activeVisualButtonOutline:setFillColor(unpack(utilities:hex2rgb('#11bbc0', 0.2)))
        local super = self
...
function _M:key(event)
    ...
    elseif event.phase == 'up' and (not event.device or event.device == self.activeDevice) then
        if self.onKeyUp then
            self.onKeyUp(event.keyName, k and k.type)
        end
    end
end
...
function _M.activate()
    ...
    local delay = 1
    if string.lower(system.getInfo('platformName')) == 'tvos' then
